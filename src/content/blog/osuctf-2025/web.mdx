...
<Chall
  ...
  flag="osu{1m_n0t_4fr4id_0f_the_b1g_bl4ck_4nym0re3}"
...
First, the circles' hit time, this is probably controlled by something of the `animation` property in `style`. Try log what that is for every circle:
...
And what controls each circle's position? That's probably `left` in `style` for most cases, in that case it can't cross `640px` and can't be lower than `0px`. Check in for that, same method:
```js
document.querySelectorAll("#n").forEach(thing => {
...
Anyway, after knowing both of those thing, we create a `boolean`, that boolean has to pass both conditions before being passed to our cheat:
```js
document.querySelectorAll("#n").forEach(thing => {
