...
```php title="login.php" startLineNumber=9 {"The password checker": 3-4}
if (
  $username == "peppy" && 
...
A tiny amount of Googling later and we'll find [this vulnerability](https://www.php.net/manual/en/function.strcmp.php#121789) with using `strcmp(){:php}` like this in `PHP`, which allows us to type juggle and get in without ever touching entropy itself. To explain what would happen if we throw an array in `strcmp(){:php}` to compare against a string, we get `NULL`, and `NULL == 0` is `true`.

Simple `DevTools` does the job well.

![DevTools](./assets/web/admin-panel/2.png)

I wonder why does the world keep using `PHP` if things like this have existed for 8 years and never got patched?
...
```php title="admin.php" startLineNumber=8 {"Very interesting...": 6-12}
...
<Chall
  ...
  category="web"
  solvedInTime="no"
  files={["web_osu-css.tar.gz"]}
  flag="osu{1m_n0t_4fr4id_0f_the_b1g_bl4ck_4nym0r3}"
...
Wowzers. The challenge author actually recreated osu! in the one and only.
...
Oh you thought.
![The anti-cheat](./assets/web/osu-css/1.png "How did they put anticheat in here.")
...
```html title="osu!css.html" startLineNumber=110 {"That's what we need":3-4}
...
Alright that works. But now we have another problem: the flag is cut off.
![The flag is cut off](./assets/web/osu-css/2.png "The flag is cut off!!!")
...
```html title="osu!css.html" startLineNumber=110 {"Man.":4-5}
<h1 style="color:#DE5B95">osu!css</h1>
  <div
    style="width: 640px; height: 480px; 
...
There's the full flag. This challenge is solved.
![The full flag](./assets/web/osu-css/3.png "The full flag")
...
```js title="scorepost.js" startLineNumber=72 {"ImageMagick is gm": 2-3}
...
One thing worth pointing out of this challenge is it's rated `1/5` by the challenge authors, but it has less solves than some `2/5`s. I didn't get to this so I don't know what happened, but for sure people might have missed a solve in plain sight.
...
<Chall
  title="Chart Viewer & Beatmap List"
  authors={["chara & strellic"]}
  category="web"
  solvedInTime="no"
>
